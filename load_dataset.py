...
def load_test_data(dataset_dir, PATCH_WIDTH, PATCH_HEIGHT, DSLR_SCALE):
    ...
    for i in range(0, NUM_TEST_IMAGES):
        ...
        I = resize(I, (int(I.shape[0]*0.5), int(I.shape[1]*0.5)))
...
def load_test_data(dataset_dir, PATCH_WIDTH, PATCH_HEIGHT, DSLR_SCALE):
    ...
    for i in range(0, NUM_TEST_IMAGES):
        ...
        I = cv2.imread(test_directory_orig + str(i) + '.jpg')
        #I_depth = cv2.imread(test_directory_orig_depth + str(i) + '.png')
        ...
        I = cv2.imread(test_directory_blur + str(i) + '.jpg')
        I = np.float32(resize(I, (I.shape[0](DSLR_SCALE / 2),I.shape[0](DSLR_SCALE / 2)))) / 255.0
        test_answ[i, :] = I
...
def load_training_batch(dataset_dir, PATCH_WIDTH, PATCH_HEIGHT, DSLR_SCALE, train_size):
    ...
    for img in TRAIN_IMAGES:
        ...
        I = cv2.imread(test_directory_orig + str(img) + '.jpg')
        I_depth = cv2.imread(test_directory_orig_depth + str(img) + '.png')
        ...
        I = cv2.imread(test_directory_blur + str(img) + '.jpg')
        I = np.float32(resize(I, (I.shape[0](DSLR_SCALE / 2),I.shape[0](DSLR_SCALE / 2)))) / 255.0
        test_answ[i, :] = I
...
def load_input_image(image_dir, depth_maps_dir, photo):
    ...
    I = cv2.imread(image_dir + photo)
    I_depth = cv2.imread(depth_maps_dir + str(photo.split(".")[0]) + '.png')
